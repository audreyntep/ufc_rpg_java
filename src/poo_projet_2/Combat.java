...
public class Combat extends Utilitaire
{
	...
	{	
		//Initialisation des propietes du combat
		this.round = 100;
		...
		//Appel de la methode pour lancer un combat
	...
	{
		//Repetition des affrontements jusqu'au dernier round ou jusqu'a la mort d'un combattant
		for(i = 1; i <= round; i++) 
		{	
			//Si les PV de l'un des combattants est a 0 => fin de la boucle combat
	...
	public void confrontation(Personnage puller, Personnage target)
	{
		...
		//Si la valeur de damage est superieure a la valeur de pv de la target
		if(damage >= target.getPv()) 
		{
			//alors on plafonne damage a la valeur de pv de la target
			damage = target.getPv();
		}
		...
		//On affiche le resultat de l'attaque et on met a jour les pv de la target
		System.out.println(puller.getNom() +" attaque "+ target.getNom() +" et lui inflige "+ damage +" points de degats.");
		...
		/*
		//On ajoute du PV � la target si c'est un mage
